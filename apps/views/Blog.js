...
export class Blog extends React.Component {
    ...
    componentDidMount() {
        return fetch('https://public-api.wordpress.com/rest/v1.1/sites/rlgdeveloperassociates.wordpress.com/posts')
        ...
        { this.state.blogLoaded && (
            <View style={{ paddingTop: 40}} >
                <FlatList
                    ...
                    keyExtractor={(item, index) => item.ID.toString()}renderItem={({item}) =>
                        <BlogItem
                            id={item.ID}
                            title={item.title}
